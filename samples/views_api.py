...
def handle_post_request(request, facilities, PastRegimensFormSet):
    ...
    if SampleService.validate_forms(patient_form, envelope_form, sample_form, drug_resistance_form, past_regimens_formset, pst):
        ...
        #    sample_form.add_error('barcode', 'An error occurred while saving the sample. Please try again.')
        #   
	...
	for sample_reception in sample_reception_objs:
		...
		if not s:
			s = Sample(locator_category = 'v',locator_position=sample_reception.barcode[8:10],
					...
					date_received = envelope.created_at,form_number=sample_reception.barcode,
		...
		if sample_reception_form.is_valid():
			...
			if s:
				...
				s = Sample(tracking_code_id = tr_code_id,locator_category = request.POST.get('locator_category'),locator_position=request.POST.get('locator_position'),
					...
					form_number=request.POST.get('barcode'),facility_id = request.POST.get('facility'),
					sample_type=request.POST.get('sample_type'),date_received=datetime.now(), envelope_id = env_id,reception_art_number=request.POST.get('reception_art_number'),facility_reference=facility_reference,facility_patient = fac_pat,verified=0)
				s.save()
...
def receive_batch(request,ret_to_fun = 0):
	...
	if request.method == 'POST' or ret_to_fun:
		...
		if saved_id:
			...
			form_number=request.POST.get('the_barcode'),reception_art_number = request.POST.get('reception_art_number'),facility_id = request.POST.get('facility'),
...
def list(request):
	...
	return render(request, 'samples/list.html', {'global_search':search_val,'is_data_entered':is_data_entered,'sample_without_results':sample_without_results,'hie_samples_pending_reception':hie_samples_pending_reception })

def update_patient_parent(request):
	parent_patients = Patient.objects.filter(is_the_clean_patient=1, facility_id=1526)
...
def remove(request, sample_id):
	...
	connections['default'].cursor().execute("UPDATE vl_worksheet_samples SET sample_id=null WHERE sample_id=%s",[sample.id])
	#now remove sample
	sample.delete() 
...
def verify_list(request):
	...
	samples = Sample.objects.filter(filters).order_by('barcode')
...
def search(request):
	...
	if switch_sample:
		...
		return render(request, 'samples/search.html', {'samples':samples, 'approvals':approvals,'remove_sample':remove_sample,'switch_sample':switch_sample})
