...
urlpatterns  = [
	...
	re_path(r'^check_facility_identifier/$', login_required(views.check_facility_identifier), name='check_facility_identifier'),
	...
	re_path(r'^tracking_codes/$', login_required(views.tracking_codes), name='tracking_codes'),
	re_path(r'^manage_envelopes/$', login_required(views.manage_envelopes), name='manage_envelopes'),
	re_path(r'^manage_envelopes_json/$', login_required(views.manage_envelopes_json), name='manage_envelopes_json'),
	re_path(r'^manage_envelope/(?P<envelope_id>[0-9]+)/$', login_required(views.manage_envelope_details), name='manage_envelope_details'),
	re_path(r'^manage_envelope/(?P<envelope_id>[0-9]+)/update/$', login_required(views.manage_envelope_update), name='manage_envelope_update'),
	re_path(r'^manage_envelope/(?P<envelope_id>[0-9]+)/delete/$', login_required(views.manage_envelope_delete), name='manage_envelope_delete'),
	re_path(r'^dr_list/$', login_required(views.dr_list), name='dr_list'),
	re_path(r'^archival_samples/$', login_required(views.archival_samples), name='archival_samples'),
	re_path(r'^archival_box_positions/$', login_required(views.archival_box_positions), name='archival_box_positions'),
	re_path(r'^dr_samples_json/$', login_required(views.dr_samples_json), name='dr_samples_json'),
	re_path(r'^dr_pending_decision_json/$', login_required(views.dr_pending_decision_json), name='dr_pending_decision_json'),
	re_path(r'^dr_requests_without_sample_json/$', login_required(views.dr_requests_without_sample_json), name='dr_requests_without_sample_json'),
	re_path(r'^lab_archival_json/$', login_required(views.lab_archival_json), name='lab_archival_json'),
	re_path(r'^create_dr_request/$', login_required(views.create_dr_request), name='create_dr_request'),
	re_path(r'^attach_dr_sample/(?P<dr_request_id>[0-9]+)/$', login_required(views.attach_dr_sample), name='attach_dr_sample'),
	re_path(r'^decide_dr_request/(?P<dr_request_id>[0-9]+)/$', login_required(views.decide_dr_request), name='decide_dr_request'),
	re_path(r'^save_lab_archival/(?P<sample_id>[0-9]+)/$', login_required(views.save_lab_archival), name='save_lab_archival'),
	re_path(r'^dr_list_export/$', login_required(views.dr_list_export), name='dr_list_export'),
	...
	re_path(r'^tracking_codes_json/$', login_required(data_table_views.tracking_codes_json), name='tracking_codes_json'),
