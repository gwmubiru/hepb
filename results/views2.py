		...
		if m_type == 'R':
			...
			for row in reader.iterrows():
				...
				if(sample_location == 'A1'):
					...
					#result_run.save()
				elif(sample_location == 'C1'):
					result_run.high_positive_ctrl = result
				#save the result and also update the identifiers

				update_sample_and_save_result(m_type,instrument_id,result, multiplier, user, test_date,result_run,index)
...
def update_sample_and_save_result(machine_type,instrument_id,result, multiplier, user, test_date,result_run,row_index):
	...
	#else:
	#	result_dict = result_utils.get_result(result, multiplier,machine_type,0, '')
