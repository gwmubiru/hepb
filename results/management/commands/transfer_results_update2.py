...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		self.worksheet = options['period'][0]
		self.old_results = []
		self.__save_results()
	...
	def __get_results(self):
		...
		sql = """SELECT s.id AS sid, rr.*, fp.*, fp.id AS fpid, rr.created AS auth_at, rr.createdby AS auth_by, w.machineType, w.createdby AS test_by
			    ...
			    WHERE rr.worksheet_id=%s
			    GROUP BY s.id"""

		cursor = connections['old_db'].cursor()
		cursor.execute(sql, [self.worksheet])
	...
	def __save_result(self, r):
		...
		if rr_result:
			sample_id = int(r.get('sid'))
			res = Result.objects.filter(sample_id=sample_id).first()
			res = res if res else Result()
			...
			fpid = r.get('fpid')
			if fpid:
				res_qc = ResultsQC.objects.filter(result=res).first()
				res_qc = res_qc if res_qc else ResultsQC()
				res_qc.result = res
				ready = r.get('ready')
				...
				if printed == 'YES' or downloaded == 'YES':
					res_dispatch = ResultsDispatch.objects.filter(sample_id=res.sample_id).first()
					res_dispatch = res_dispatch if res_dispatch else ResultsDispatch()
